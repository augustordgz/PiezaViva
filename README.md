...
## ⚠️ Pendiente: colocar las imágenes reales

Es lo **único** que falta para que el sitio esté 100% terminado. Todos los `<img>` ya están en el código, solo hay que subir los archivos con estos nombres exactos a la carpeta correspondiente:

Mientras no estén subidas, el sitio funciona igual — simplemente se ve el ícono de "imagen rota" del navegador en su lugar.
4. **WhatsApp**: confirmar que el número en el botón flotante sea el definitivo.
5. Subir todo por FTP o el Administrador de Archivos de cPanel a `public_html/`.
5. Subir todo por FTP o el Administrador de Archivos de cPanel a `public_html/`.
